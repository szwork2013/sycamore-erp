...
var QuoteActions = {
	saveQuote: function saveQuote(quote) {
		if (typeof quote._id != "undefined" && quote._id != null) {
			// POST
			Api.postQuote(quote._id, quote, function (response) {
			...
			// PUT
			Api.putQuote(quote, function (response) {
				AppDispatcher.handleViewAction({
					actionType: QuoteConstants.UPDATE_QUOTE,
					quote: response.body
				});
	...
	addProductToQuote: function addProductToQuote(product) {
		AppDispatcher.handleViewAction({
			actionType: QuoteConstants.ADD_PRODUCT_TO_QUOTE,
			product: product
		});
	},
	setCustomerOnQuote: function setCustomerOnQuote(value, selectedOptions) {
	...
	setProductQuantityOnQuote: function setProductQuantityOnQuote(productIndex, event) {
		AppDispatcher.handleViewAction({
	...
	setPropertyOnQuote: function setPropertyOnQuote(value, selectedOptions) {
