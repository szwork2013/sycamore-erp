...
var Api = {
	...
	getProducts: function getProducts(queryOptions, callback) {
		...
		d.run(function () {
			...
			request.get("/sycamore-erp/products.json").query(queryOptions).set("Accept", "application/json").end(d.intercept(callback));
		});
	},
	...
	putQuote: function putQuote(quote, callback) {
		var d = domain.create();

		d.on("error", this.handleError);

		d.run(function () {
			request.put("/sycamore-erp/quote").send({ quote: quote }).set("Accept", "application/json").set("Content-Type", "application/json").end(d.intercept(callback));
		});
	},
	...
	postQuote: function postQuote(id, quote, callback) {
		var d = domain.create();

		d.on("error", this.handleError);

		d.run(function () {
			request.post("/sycamore-erp/quote/" + id).send({ quote: quote }).set("Accept", "application/json").set("Content-Type", "application/json").end(d.intercept(callback));
		});
	},
