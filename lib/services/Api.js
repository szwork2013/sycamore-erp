...
var Api = {
	...
	putProductVariation: function putProductVariation(product, callback) {
		var d = domain.create();

		d.on("error", this.handleError);

		d.run(function () {
			request.put("/sycamore-erp/productVariation").send({ product: product }).set("Accept", "application/json").set("Content-Type", "application/json").end(d.intercept(callback));
		});
	},
	putProductVariationGroup: function putProductVariationGroup(product, callback) {
		var d = domain.create();

		d.on("error", this.handleError);

		d.run(function () {
			request.put("/sycamore-erp/productVariationGroup").send({ product: product }).set("Accept", "application/json").set("Content-Type", "application/json").end(d.intercept(callback));
		});
	},
