...
var View = React.createClass({
	...
	selectRow: function selectRow(id) {
		window.location.href = this.props.locals.applicationUrl + "productVariationGroup/" + id;
	},
	render: function render() {
		return React.createElement(
			...
			React.createElement(
				...
				React.createElement(
					"a",
					{ href: this.props.locals.applicationUrl + "productVariationGroup", className: "right fancy radius button tiny" },
					React.createElement("i", { className: "in-button-icon fa fa-fw fa-plus" }),
					" Create"
				)
			),
			React.createElement(List, { apiUrl: this.props.locals.applicationUrl + "productVariationGroups.json", list: this.props.locals.list, selectRow: this.selectRow, updateListener: this._onUpdate })
		);
	}
});
