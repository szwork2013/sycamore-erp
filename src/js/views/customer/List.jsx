		...
		return (
			<Layout pageTitle={this.props.locals.list.title} locals={this.props.locals}>
				<ActionsBar pageTitle={this.props.locals.list.title}>
					<SettingsButton />
					<a href={this.props.locals.applicationUrl + "customer"} className="right fancy radius button tiny">
						<i className="in-button-icon fa fa-fw fa-plus"></i> Create
					</a>
				</ActionsBar>
				<List apiUrl={this.props.locals.applicationUrl + "customers.json"} list={this.props.locals.list} selectRow={this.selectRow} updateListener={this._onUpdate} />
			</Layout>
		);
	}
});
